        ...
        if not attrs["_abstract"]:
            ...
            if module_name.startswith("rodan.jobs."):
                ...
                if settings.TEST and module_name == "rodan.test.dummy_jobs":
                    ...
                    raise ValueError(
                        "Invalid use of Rodan jobs - job package must locate in /rodan/jobs/"
                    )
            ...
            if not Job.objects.filter(name=attrs["name"]).exists():
                ...
                try:
                    for ipt in attrs["input_port_types"]:
                        ...
                        if len(resource_types) == 0:
                            raise ValueError(
                                "No available resource types found for this InputPortType: {0}".format(
                                    ipt["resource_types"]
                                )
                            )
                    ...
                    for opt in attrs["output_port_types"]:
                        ...
                        if len(resource_types) == 0:
                            raise ValueError(
                                "No available resource types found for this OutputPortType: {0}".format(
                                    opt["resource_types"]
                                )
                            )
                ...
                def check_field(
                    ...
                    if not compare_fn(original_value, new_value):
                        if not UPDATE_JOBS:
                            raise ImproperlyConfigured(
                                "The field `{0}` of Job `{1}` seems to be updated: {2} --> {3}. Try to run `manage.py migrate` to confirm this update.".format(
                                    field_name,
                                    j.name,
                ...
                def check_port_types(which):
                    ...
                    for pt in db_pts:
                        ...
                        if (
                            ...
                            if attrs_pt["minimum"] != pt.minimum:
                                if not UPDATE_JOBS:
                                    raise ImproperlyConfigured(
                                        "The field `{0}` of {5} Port Type `{1}` of Job `{2}` seems to be updated: {3} --> {4}. Try to run `manage.py migrate` to confirm this update.".format(
                                            "minimum",
                                            pt_name,
                                            j.name,
                                            pt.minimum,
                            ...
                            if not UPDATE_JOBS:
                                raise ImproperlyConfigured(
                                    "The {2} Port Type `{0}` of Job `{1}` seems to be deleted. Try to run `manage.py migrate` to confirm this deletion.".format(
                                        pt_name, j.name, msg
                                    )
                                )
                            else:
                                confirm_delete = confirm(
                                    "The {2} Port Type `{0}` of Job `{1}` seems to be deleted. Confirm (y/N)? ".format(
                                        pt_name, j.name, msg
                                    )
                                )
                                if confirm_delete:
                    ...
                    if (
                        ...
                        for pt in attrs_pts:
                            if not UPDATE_JOBS:
                                raise ImproperlyConfigured(
                                    "The {2} Port Type `{0}` of Job `{1}` seems to be newly added. Try to run `manage.py migrate` to confirm this update.".format(
                                        pt["name"], j.name, msg
                                    )
                                )
                            else:
                                confirm_update = confirm(
                                    "The {2} Port Type `{0}` of Job `{1}` seems to be newly added. Confirm (y/N)? ".format(
                                        pt["name"], j.name, msg
                                    )
                                )
                                if confirm_update:
                                    ...
                                    if len(resource_types) == 0:
                                        raise ValueError(
                                            "No available resource types found for this {1}PortType: {0}".format(
                                                pt["resource_types"], msg
                                            )
                                        )
                                    i.resource_types.add(*resource_types)
                                    print("  ..updated.\n\n")
...
class RodanTask(Task, metaclass=RodanTaskType):
        ...
        for input in input_objs:
            ...
            if input.resource is not None:  # If resource
                ...
                inputs[ipt_name].append(
                    [
                        _extract_resource(x, input.resource_list.resource_type.mimetype)
                        for x in input.resource_list.resources.all()
                    ]
                )
            else:
                raise RuntimeError(
                    "Cannot find any resource or resource list on Input {0}".format(
                        input.uuid
                    )
                )
        return inputs
    ...
    def _outputs(self, runjob):
        ...
        for output in output_objs:
            ...
            if output.resource is not None:  # If resource
                ...
                raise RuntimeError(
                    "Cannot find any resource or resource list on Output {0}".format(
                        output.uuid
                    )
                )
        return outputs
                    ...
                    for output in output_list:
                        if output["is_list"] is False:
                            if not os.path.isfile(output["resource_temp_path"]):
                                raise RuntimeError(
                                    "The job did not produce the output file for {0}".format(
                                        opt_name
                                    )
                                )
                        else:
                            ...
                            if len(files) == 0:
                                raise RuntimeError(
                                    "The job did not produce any output files for the resource list for {0}".format(
                                        opt_name
                                    )
                                )
    ...
    def _add_error_information_to_runjob(self, exc, einfo):
        ...
        try:
            ...
            logger.warning(
                "The my_error_information method is not implemented properly (or not implemented at all). Exception: "
            )
