    ...
    if mimetype not in registered_rts:  # If not yet exist in DB:
        if not UPDATE_JOBS:
            raise ImproperlyConfigured(
                "The catalogue of local ResourceTypes does not match the ones in database: local ResourceType `{0}` has not been registered. Please run `manage.py migrate` on Rodan server to update the database."
            )
        else:
...
if registered_rts:  # if there are still registered ones
    if not UPDATE_JOBS:
        raise ImproperlyConfigured(
            "The following ResourceTypes are in database but not registered in the code. Perhaps they have been deleted in the code but not in the database. Try to run `manage.py migrate` to confirm deleting them:\n{0}".format(
        ...
        for mimetype, info in registered_rts.items():
            confirm_delete = input(
                "ResourceType `{0}` is in database but not registered in the code. Perhaps it has been deleted in the code but not yet in the database. Confirm deletion (y/N)? ".format(
                    mimetype
                )
            )
            if confirm_delete.lower() == "y":
                try:
                    ...
                    confirm_delete = input(
                        "  ..not deleted because of an exception: {0}. Perhaps there are Resources or ResourceLists using this ResourceType. Confirm deletion of related Resources (y/N)? ".format(
                            str(e)
                        )
                    )
                    if confirm_delete.lower() == "y":
        ...
        for j_name in job_list:
            ...
            if confirm_delete.lower() == "y":
                try:
                    ...
                    confirm_delete = input(
                        "  ..not deleted because of an exception: {0}. Perhaps there are WorkflowJobs using this Job. Confirm deletion of related WorkflowJobs (y/N)? ".format(
                            str(e)
                        )
                    )
                    if confirm_delete.lower() == "y":
