...
def update_database_trigger(sender, **kwargs):
    ...
    try:
        ...
        if "superuser" in str(e):
            ...
            while not solved:
                ...
                if (
                    "RODAN_PSQL_SUPERUSER_USERNAME" in os.environ
                    and "RODAN_PSQL_SUPERUSER_PASSWORD" in os.environ
                ):
                    print(
                        "Environment variables RODAN_PSQL_SUPERUSER_USERNAME and RODAN_PSQL_SUPERUSER_PASSWORD detected."
                    )
                    choice = "1"
                elif "RODAN_PSQL_SUPERUSER_COMMAND" in os.environ:
                    print(
                        "Environment variables RODAN_PSQL_SUPERUSER_COMMAND detected."
                    )
                    choice = "2"
                ...
                while choice not in ("1", "2"):
                    ...
                    print(
                        "Rodan needs PostgreSQL superuser permission to proceed. Options:"
                    )
                    print("  1. Provide the username and password of a superuser.")
                    print(
                        "  2. Provide the shell command to log in PostgreSQL console as a superuser (typically `sudo -u postgres psql --dbname={0}`)".format(
                            settings.DATABASES["default"]["NAME"]
                        )
                    )
                    print(
                        "(Please inform Rodan developers if there is another way of connecting to PostgreSQL.)"
                    )
                    choice = input("Choice: ")
    ...
    try:
        ...
        if "No module named redis" in str(e):
            ...
            print("================================================================")
            print(
                "Please execute `pip install redis` as a system package (not in virtualenv) on the database server {0}.".format(
            ...
            print("================================================================")
            print(
                "Please start redis-server at {0}:{1}.".format(
                    settings.WS4REDIS_CONNECTION["host"],
                    settings.WS4REDIS_CONNECTION["port"],
                )
            )
    ...
    print("Registering Rodan database triggers...", end=" ")
...
def assign_perms_user_userpreference(
    sender, instance, created, raw, using, update_fields, **kwargs
):
    if created:
        model_name = sender._meta.model_name
