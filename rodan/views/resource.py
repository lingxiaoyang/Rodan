...
class ResourceList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if wfrun_uuid:
            condition &= Q(origin__run_job__workflow_run__uuid=wfrun_uuid) & (
                Q(inputs__isnull=True)
                | ~Q(inputs__run_job__workflow_run__uuid=wfrun_uuid)
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        for fileobj in request.data.getlist("files"):
            ...
            resource_obj = serializer.save(
                name=filename_without_ext, creator=request.user
            )
            resource_obj.resource_file.save(
            ...
            registry.tasks["rodan.core.create_resource"].si(
    ...
    def get(self, request, resource_uuid, working_user_token, *a, **k):
        ...
        if viewer == "diva":
            return render(
                ...
                "diva.html",
                {
                    "page_title": "View Resource: {0}".format(
                        resource.name or resource.pk
                    ),
