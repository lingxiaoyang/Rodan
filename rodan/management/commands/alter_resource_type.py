...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        rt_table = [["name", "description", "extension"]]
        ...
        while True:
            print("")
            rt_mimetype = input(
                "Type the name of the ResourceType that you would like to alter: "
            )
            try:
        ...
        print("")
        description = input(
            "Type the new description for {0} ({1}): ".format(
                rt.mimetype, rt.description
            )
        )
        rt.description = description or rt.description
        extension = input(
            "Type the new extension for {0} ({1}): ".format(rt.mimetype, rt.extension)
        )
